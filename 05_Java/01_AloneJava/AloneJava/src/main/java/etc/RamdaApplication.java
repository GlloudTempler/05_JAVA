...
/**
 * packageName : etc
 * fileName : RambdaApplication
 * author : GGG
 * date : 2023-10-02
 * description : 람다식 (js:화살표함수) : 함수형 인터페이스로 사용
 * 요약 :
 * <p>
 * ===========================================================
 * DATE            AUTHOR             NOTE
...
//        TODO: 여기 부터 코딩
//              js : 함수의 매개변수로 함수를 넣을 수 있음, 리턴값도 함수 내보내기 기능
//              java : 함수의 매개변수/리턴값으로 함수 사용 금지(기본)
//              => 부가기능 추가 : 5가지 경우의 대표 함수를 만들어서 사용
//              Runnable 함수 : 매개변수(x), 리턴값(x) 함수 ex) void run()
//              Runnable(실행) 함수 : 매개변수(x), 리턴값(x) 함수 ex) void run()
//              Supplier(공급자) 함수 : 매개변수(x), 리턴값(o) 함수 ex) int get()
//              Consumer(소비자) 함수 : 매개변수(o), 리턴값(x) 함수 ex) void accept(int t)
//              Function(일반) 함수 : 매개변수(o), 리턴값(o) 함수 ex) int apply(int t)
//              Predicate(논리(명제)) 함수 : 매개변수(o), 리턴값(o, 참/거짓) 함수 ex) boolean apply(int t)
        Supplier<Integer> s = () -> (int)(Math.random()*100) + 1; // 1~100까지의 랜덤 수
        Consumer<Integer> c = i -> System.out.println(i + ","); // i 출력 함수
        Predicate<Integer> p = i -> i%2 == 0; // 2의 배수만 리턴하는 함수
        Function<Integer, Integer> f = i -> i/10*10; // i의 값 중 1의 자리를 없애는 함수 // ex) 9.7 -> 90
//        결과 실행
        List<Integer> list = new ArrayList<>();
//        TODO : 정적함수를 호출하면 마지막 매개변수(배열)에 값이 저장되어 리턴됨(화면출력됨)
        makeRandomList(s, list);
        System.out.println(list); // 결과 출력 : 1~100까지 랜덤 수가 화면 출력
        System.out.println("---------------------------------");
        printEventNum(p,c,list); // 결과 : 배열에 짝수인 것만 화면에 출력
        System.out.println("---------------------------------");
        List<Integer> list2 = doSomething(f, list);
        System.out.println(list2); // 결과 : 1의 자리를 없앤 배열
